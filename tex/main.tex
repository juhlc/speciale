...
We choose here a leave-one-out approach and forecasts two periods into the future:
\begin{center}
	\begin{figure}[!ht]
	...
	\end{figure}
\end{center}
For anchovy, an embedding dimension of $3$ seems fine, for temperature $4$ seems fine, and for sardine it is less clear what is appropriate. In this case, 4 would be appropriate for all 3 cases.\\
More sophisticated methods do exist, we sketch another one here:
\begin{enumerate}[label=(\roman*)]
	\item 
\end{enumerate}
...
We then move to the prediction of $Y(t)$ for $t\geqslant 1+(E-1)\tau$:
\begin{enumerate}[label=(\roman*)]
	\item First we take $t_1,\ldots,t_{E+1}$ to be the $E+1$ numbers in succession (closest to farthest) for which $\norm{x(t)-x(t_{i})}$ is minimized (randomized order in case of a tie).
	\item We then compute the score of each chosen element
	$$u_i=\exp\left(-\frac{\norm{x(t)-x(t_i)}}{\norm{x(t)-x(t_1)}}\right)$$
