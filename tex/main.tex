...
In this short chapter, we will consider the Convergent Cross Mapping (CCM) approach of \cite{Sugihara} and try to connect it to the theoretical work we have undertaken so far. This chapter is very different because we will not develop any new theory, only briefly considering the practical complications. We will also formulate us in manifolds a lot of the time, but as we saw in the previous chapter, the attractor versions also deal with manifolds as quite an integral part of the statements. We will now approach the problem of causal inference in our dynamical systems setting. In its most simple form, we imagine that we have made observations from two-time series in the window $1,\ldots, T$:
\begin{align*}
X&=(X(1),\ldots, X(T)),\\
Y&=(Y(1),\ldots, Y(T)).
\end{align*}
For a fixed embedding dimension $E_X$, we can now form the lagged coordinate vectors of $x$ for a delay size $\tau$, that we will often set $\tau=1$:
$$x(t)=(X(t), X(t-\tau),X(t-2\tau),\ldots, X(t-(E_X-1)\tau))$$
for $t=1+(E_X-1)\tau$ to $t=T$. We denote by
...
And a corresponding shadow manifold of $Y$:
$$M_Y\coloneqq\{y(t)\mid t\in \{1+(E_Y-1)\tau,\ldots,T\}\}$$
 This approach raises a few questions: How do we choose $E$, convert the theoretical results into something we can test, and address the assumptions described at the end of the last chapter?\\[5pt]
In this chapter, we will answer these questions one by one and use the techniques in the introductory example of this thesis. However, it is crucial to remember that the theory of the previous chapter applies only to purely deterministic systems, and real-world examples often have a mix of deterministic and stochastic structures. It is outside the scope of this thesis to tackle this problem. Before that, we return to the California Current example with the following time-series of the introduction:
\begin{center}
	\begin{figure}[!ht]
		\includegraphics[width=\textwidth]{{"../r code/sardine"}.png}
		\caption{}
	\end{figure}
\end{center}
We discussed that they exhibited traits of non-linear deterministic systems, but now we shall analyze them further.
...
\begin{center}
	\begin{figure}[!ht]
		\includegraphics[width=\textwidth]{{"../r code/delayemb"}.png}
		\caption{}
	\end{figure}
\end{center}
For anchovy, an embedding dimension of three seems fine, for temperature four seems fine, and for sardine, it is less clear what is appropriate. In this case, four is probably appropriate for all three cases. More sophisticated methods exist; we sketch such a method of \cite{cao} here. We introduce the following notation: Let $x_E(t)$ be the delay embedding for embedding dimension $E$ and let $n(t, E)$ be such that $x_{E}(n(t,E))$ is the closet neighbor of $x_E(t)$.
\begin{enumerate}[label=(\roman*)]
	\item  For each $i=1,2,\ldots,T-E\tau$, we compute
	$$a(t,E)=\frac{\norm{x_{E+1}(t)-x_{E+1}(n(t,E))}}{\norm{x_{E}(t)-x_{E+1}(n(t,E)}}$$
	\item Compute
	$$f(E)=\frac{1}{T-E\tau}\sum_{i=t}^{T-E\tau}a(t,E),$$
	and compute then
	$$f1(E)=\frac{f(E+1)}{f(E)}$$
	\item Furthermore, we compute
	$$f^*(E)=\frac{1}{T-E\tau}\sum_{t=1}^{T-E\tau}\norm{x_{t+E\tau}-x_{n(t,E)+E\tau}}$$
	and compute then
	$$f2(E)=\frac{f^*(E+1)}{f^*(E)}$$
\end{enumerate}
The reasoning behind this is that once we reach a dimension sufficient for an embedding, then the shadow manifolds for all dimension sizes bigger should all be homeomorphic. Therefore, neighbors in a lower-dimensional embedding should still be neighbors in a higher dimension. In practice, one often observes that $f_1$ increases to the minimum dimension and then stagnates in an attractor setting. As for $f2$, it also measures whether the data is deterministic - if it is, then the curve cannot be constant. We get the following results:\\\\
\begin{figure}[H]
    \centering
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=\textwidth]{{"../r code/sardines"}.png} % first image
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=\textwidth]{{"../r code/anchovy"}.png} % second image
    \end{minipage}
\end{figure}
\begin{figure}[H]
 \includegraphics[width=0.5\textwidth]{{"../r code/temp"}.png} 
 \caption{}
\end{figure}
We see that the $f2$ curves (called $E2$ on the graphs) are pretty flat. However, they do deviate from a constant 1, so it is reasonable to expect some deterministic structure. Moreover, we see that a dimension size of 4 across the board is not too unreasonable. However, it is inconclusive - the \enquote{accurate} dimension could be much higher; maybe 8 is also reasonable. The green interval is for $f1$ (called $E1$ on the graphs). We proceed with $E=4$ and remark that if we were modeling with a DCM, we could potentially have recovered more \enquote{variables} than we have observed.
...
\begin{figure}[H]
	\includegraphics[width=\textwidth]{{"../r code/p1"}.png} 
\end{figure}
We see that this confirms our findings in the intro, namely that there seems to be a causal influence from SST to sardine and anchovy in this case - this was not supported by \cite{Sugihara}, but the causal direction does go the same way. In the opposite direction, do we not find any support for a causal direction.\\\\
To use the theory we have developed, we need to address the assumptions at the end of the last chapter. The assumptions about the SCM approach are something other than what we usually verify from a concrete set of observations - not in the stochastic setting either. As described at the end of the last chapter, the rest of the assumptions can take much work to deal with in practice. We can consider the non-linearity of the observations as a way to test whether it is appropriate to fit it with a dynamical systems approach. It is suggested by \cite{SMap} to use the S-map approach, also called the sequentially locally weighted global linear map approach, with more details in the work of \cite{EDM}. We already covered a way to test for deterministic behavior and many more. We do not go into depth with these methods, but we mention their existence. Moreover, we also refer to \cite{Ye2015} and \cite{Leng2020}, which address problems not resolved by CCM, respectively, that of distinguishing bidirectional causality and unidirectional causality in systems with some degree of synchronicity and identifying the causal chain in a system knowing the causal directions between causally related variables. However, neither of these is directly covered by the methods we developed in this thesis.
